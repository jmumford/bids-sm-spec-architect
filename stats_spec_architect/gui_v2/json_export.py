            ...
            if key == 'transformations':
                ...
                if widget is not None and isinstance(widget, dict):
                    ...
                    for model_key, model_widget in widget.items():
                        field_name, value = _process_widget(model_key, model_widget)
                        if value is not None:
                ...
                if widget is not None and isinstance(widget, dict):
                    ...
                    for dummy_key, dummy_widget in widget.items():
                        field_name, value = _process_widget(dummy_key, dummy_widget)
                        if value is not None:
...
def _extract_transformations(transformation_widget):
    ...
    if 'Transformer (req)' in transformation_widget.widget_output:
        transformer = transformation_widget.widget_output['Transformer (req)'].get()
        if transformer:
...
def _process_transformation_field(field_name, widget, transform_name):
    ...
    try:
        from stats_spec_architect.gui_v2.model_discovery import (
...
def _process_widget(label, widget):
    """
    Process a widget to extract its value.

    Args:
        label: Widget label (e.g., "Name (req, str)")
        widget: Tkinter widget or value

    Returns:
        tuple: (field_name, processed_value)
    """
    # Parse label to get field name and type
